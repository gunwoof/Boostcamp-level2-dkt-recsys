...
def feat_user_correct_stats(df : pd.DataFrame, static : str) -> pd.DataFrame:
    """
    #### 사용자별 정답에 대한 평균/중간값/표준편차 등
    
    Args:
        df (pd.DataFrame): DataFrame
        static (str): [mean, median, std]
...
def feat_elapsed(df : pd.DataFrame) -> pd.DataFrame:
    ...
    # 문제풀이 시간이 전체 기준 80%가 초과하면
    # (카테고리별 풀이시간과 + 사용자별 풀이시간 / 2) 시간으로 계산
    
    # 80%의 사용자 중 풀이시간이 가장 큰 시간
    top_80p_user_max_solve_time = int(df['elapsed'].sort_values().iloc[int(0.8 * df['elapsed'].count())])
    # 전체 태그별 풀이시간의 중간값 계산
    KnowledgeTag_elapsed_mean_df = df[df['elapsed'] <= top_80p_user_max_solve_time].groupby('KnowledgeTag')['elapsed'].agg('median')
    df['KnowledgeTag_elapsed_mean'] = df['KnowledgeTag'].map(KnowledgeTag_elapsed_mean_df)
    
    # 사용자별 풀이 시간을 고려하여 평균값 계산
    mean_elapsed_df = df[df['elapsed'] <= top_80p_user_max_solve_time].groupby(['userID'])['elapsed'].mean().reset_index()
    mean_elapsed_df.columns = ['userID', 'user_elapsed_mean']
    
    # 전체 태그별 풀이시간, 사용자별 풀이시간 merge
    df = df.merge(mean_elapsed_df, how='inner', on=['userID'])
    
    # 기준 시간 80%를 초과하면 (카테고리별 풀이시간과 + 사용자별 풀이시간 / 2) 시간으로 계산
    df.loc[df['elapsed'] > top_80p_user_max_solve_time, 'elapsed'] = df.apply(lambda x:
        (x['KnowledgeTag_elapsed_mean'] + x['user_elapsed_mean']) / 2 if x['elapsed'] > top_80p_user_max_solve_time else x['elapsed'], axis=1)

    return df.drop(['KnowledgeTag_elapsed_mean', 'user_elapsed_mean'], axis=1)
...
def feat_elapsed_pca(df : pd.DataFrame) -> pd.DataFrame:
    ...
    X = df[['elapsed', 'elapsed_cumsum', 'normalized_elapsed', 'cat_elapsed_type_mean', 'relative_elapsed']]
